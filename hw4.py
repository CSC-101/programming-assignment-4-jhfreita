...
# Finally, there is error control, which I had to do some outside research to find syntax that would report where an
# error occurred and continue onto the next line.
with open(sample_file, "r") as file:
    ...
    if first_line[:16] == "population-total":
        ...
        try:
            ...
            print("An error occurred in line", traceback.extract_tb(e.__traceback__)[-1].lineno)
        ...
        try:
            ...
            error_description = traceback.format_exc()
            print("An error occurred in line", traceback.extract_tb(e.__traceback__)[-1].lineno)
    elif first_line[:12] == "filter-state":
        try:
    ...
    for line in file:
            try:
                ...
                if line[:16] == "population-total":
                    ...
                    if part1 == "percent":
                        print("2014", part2, ".", part3, ":",
                              100 * population_by_attribute(data, part4, part3) / population_total(data))
            except Exception as e:
                error_description = traceback.format_exc()
                print("An error occurred in line", traceback.extract_tb(e.__traceback__)[-1].lineno)
                continue
